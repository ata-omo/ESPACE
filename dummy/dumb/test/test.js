...
describe("Responsive Form", function () {
  ...
  for (const screen of Object.keys(screenSizes)) {
    describe(screen, () => {
      ...
      it.each(fields)(
        'should position input "%s" correctly',
        async (placeholder) => {
          const container = await driver.findElement(By.tagName("form"));
          const templateContainer = await driver.findElement(
            By.className("template-form")
          );

          const input = await driver.findElement(
            By.css(`form [placeholder="${placeholder}"]`)
          );
          const templateInput = await driver.findElement(
            By.css(`.template-form [placeholder="${placeholder}"]`)
          );
          ...
          expect(
            Math.abs(
              templateInputRect.y -
                templateContainerRect.y -
                (inputRect.y - containerRect.y)
            )
          ).toBeLessThanOrEqual(epsilon);
          expect(
            Math.abs(
              templateInputRect.x -
                templateContainerRect.x -
                (inputRect.x - containerRect.x)
            )
          ).toBeLessThanOrEqual(epsilon);

          expect(
            Math.abs(templateInputRect.width - inputRect.width)
          ).toBeLessThanOrEqual(epsilon);
          expect(
            Math.abs(templateInputRect.height - inputRect.height)
          ).toBeLessThanOrEqual(epsilon);
        }
      );

      it.each(fields)(
        'should have correct input "%s" width',
        async (placeholder) => {
          const input = await driver.findElement(
            By.css(`form [placeholder="${placeholder}"]`)
          );
      ...
      it("should position button correctly", async () => {
        ...
        const templateButton = await driver.findElement(
          By.css(`.template-form button`)
        );
        ...
        expect(
          Math.abs(
            templateButtonRect.y -
              templateContainerRect.y -
              (buttonRect.y - containerRect.y)
          )
        ).toBeLessThanOrEqual(epsilon);
        expect(
          Math.abs(
            templateButtonRect.x -
              templateContainerRect.x -
              (buttonRect.x - containerRect.x)
          )
        ).toBeLessThanOrEqual(epsilon);

        expect(
          Math.abs(templateButtonRect.width - buttonRect.width)
        ).toBeLessThanOrEqual(epsilon);
        expect(
          Math.abs(templateButtonRect.height - buttonRect.height)
        ).toBeLessThanOrEqual(epsilon);
      });
    });
  }
